...
const ContextMenu = ({ currentPlaylistId }) => {
  ...
  const handleLikeTrack = e => {
    console.log(e.target)

    $.ajax({
      url: `https://api.spotify.com/v1/me/tracks?ids=${selectTrack.replace(
        'spotify:track:',
        ''
      )}`,
      headers: {
    ...
    >
      {showMenu && (
        <ul className='context-menu'>
          ...
          <li
            className='btn-add-to-playlists'
            onMouseEnter={() => {
          ...
          >
            <span>Ajouter aux playlists</span>
            <BsFillCaretRightFill />
          </li>
          <li onClick={handleLikeTrack}>
            <span>Sauvegarder dans Titres likés</span>
          </li>
          <li onClick={handleDeleteTrack}>
              ...
              onClick={e => {
                if (e.target.localName !== 'input') {
                  ...
                  if (checkboxElement.checked == true) {
                    setListSelectPlaylists(
                      listSelectPlaylists.concat(playlistId)
                    )
                  } else {
                    setListSelectPlaylists(
                      listSelectPlaylists.filter(item => item !== playlistId)
                    )
                  }
                }
              }}
